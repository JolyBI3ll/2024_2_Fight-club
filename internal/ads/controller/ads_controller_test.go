...
func TestAdHandler_GetAllPlaces(t *testing.T) {
	...
	mockGrpcClient := &mocks.MockClient{
		MockGetAllPlaces: func(ctx context.Context, req *gen.AdFilterRequest, opts ...grpc.CallOption) (*gen.GetAllAdsResponseList, error) {
			return &gen.GetAllAdsResponseList{
				Housing: []*gen.GetAllAdsResponse{
					{Id: "1", CityName: "Test City", Address: "Test Address", RoomsNumber: 5},
				},
			}, nil
		},
	}
	...
	handler := NewAdHandler(mockGrpcClient, mockSession, mockJwtToken)

	// Создание запроса HTTP
	req, err := http.NewRequest("GET", "/api/ads/?location=Test+City&rating=5&new=true&gender=any&guests=2&limit=10&offset=0&dateFrom=2023-01-01&dateTo=2023-01-10", nil)
